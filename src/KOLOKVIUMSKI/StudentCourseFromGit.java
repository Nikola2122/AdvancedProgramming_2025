...
class ExecuteAndSort{
    public static <E extends Comparable<E>> List<E> execute(List<E> list, Function<E,E> function){
        return list.stream().map(function).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    }
}

public class StudentCourseFromGit {
    public static void main(String[] args) {
        ...
        if (testCase == 1) { // students
            ...
            if (studentScenario == 1) {
                ...
                students = ExecuteAndSort.execute(students, (s)->{
                    s.grades = s.grades.stream().map(i -> 11 - i).collect(Collectors.toList());
                    return s;
                });
                System.out.println(students);
            ...
            integers = ExecuteAndSort.execute(integers, i -> i < 100 ? i * 10 : i * 2);
