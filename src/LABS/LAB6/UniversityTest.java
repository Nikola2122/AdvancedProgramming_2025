...
class University {
    ...
    public List<String> getAllCourseNames() {
        return departments.stream().
                flatMap(d -> d.getCourses().stream()).map(Course::getName).
    ...
    public List<Course> getCoursesWithMinCredits(int minCredits) {
        return departments.stream().
                ...
                filter(c -> c.getCredits() >= minCredits).collect(Collectors.toList());
    }
    ...
    public double getAverageEnrollmentPerCourse() {
        return departments.stream().
                flatMap(d -> d.getCourses().stream()).
                mapToInt(Course::getEnrolledStudents).average().orElse(0);
    }

    public List<String> getSortedCourseCodes() {
        return departments.stream().
                flatMap(d -> d.getCourses().stream()).
                map(Course::getCode).sorted().collect(Collectors.toList());
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        return departments.stream().max(Comparator.
                comparing(d -> d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()));
    }

    public Map<Integer, Integer> getStudentsByDifficulty() {
        return departments.stream().flatMap(d -> d.getCourses().stream()).collect(Collectors.groupingBy(
                Course::getDifficulty,
    ...
    public List<String> getPopularCourseCodes(int minStudents) {
        return departments.stream().flatMap(d -> d.getCourses().stream()).
                filter(c -> c.getEnrolledStudents() >= minStudents).map(Course::getCode).collect(Collectors.toList());
    }
    ...
    public List<Course> getTop3HardestCourses() {
        return departments.stream().flatMap(d -> d.getCourses().stream()).
                sorted(Comparator.comparing(Course::getDifficulty, Comparator.reverseOrder())).limit(3).collect(Collectors.toList());
    }
    ...
    public Map<String, Double> getAverageDifficultyPerDepartment() {
        return departments.stream().collect(Collectors.toMap(
                Department::getName,
                d -> d.getCourses().stream().mapToDouble(Course::getDifficulty).average().getAsDouble()
        ));
    }
    ...
    public University mergeFourSmallestDepartments() {
        ...
        List<Department> updated = Stream.concat(
                ...
                // keep all departments NOT in the smallest 4
                departments.stream()
                        .sorted(Comparator.comparing(
                                d -> d.getCourses().stream().mapToInt(Course::getEnrolledStudents).sum()
                        ))
    ...
    int getTotalCredits(){
        return departments.stream().flatMap(d -> d.getCourses().stream()).mapToInt(Course::getCredits).sum();
    }
