...
class EmergencyRoom {
    ...
    public void mutate(Consumer<Doctor> mutator) {
        Arrays.stream(doctors).forEach(mutator);
    }
    ...
    public static void main(String[] args) {
        ...
        IntStream.range(0,n).forEach(i->{
            er.treat(()->{
                ...
                return new Doctor(Integer.parseInt(arr[0]),arr[1],Integer.parseInt(arr[2]),Integer.parseInt(arr[3]));
            });
        });
        ...
        DoctorEvaluator h1 = (d)->d.getPatients()>20;
        HighExpertiseEvaluator ex = new HighExpertiseEvaluator();

        Doctor [] passing = er.evaluate((d)->h1.evaluate(d) && ex.evaluate(d));
        ...
        String [] l = er.mapToLabels(d->String.format("Name: %s, Level: %d",d.getName(),d.getLevel()));
        String str = Arrays.stream(l).collect(Collectors.joining("\n"));
