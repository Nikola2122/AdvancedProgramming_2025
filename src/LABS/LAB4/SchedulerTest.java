...
class Scheduler<T> {
    ...
    public T next() {
        Date now = new Date();
        Map.Entry<Date, T> entry = map.ceilingEntry(now);// најмал клуч >= now
        return entry != null ? entry.getValue() : null;
    }

    // Последен елемент (во минатото)
    public T last() {
        Date now = new Date();
        Map.Entry<Date, T> entry = map.floorEntry(now); // најголем клуч <= now
        return entry != null ? entry.getValue() : null;
    }

    ArrayList<T> getAll(Date begin,Date end){
        return this.map.entrySet().stream().
                filter(e -> e.getKey().toInstant().isAfter(begin.toInstant()) && e.getKey().toInstant().isBefore(end.toInstant())).
                map(e -> e.getValue()).
