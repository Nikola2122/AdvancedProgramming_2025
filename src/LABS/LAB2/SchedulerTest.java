...
class Scheduler <T>{
    ...
    Timestamp<T> next(){
        ...
        Optional<Timestamp<T>> t = list.stream().filter(i -> i.getTime().isAfter(now)).sorted().findFirst();
        if (t.isPresent()) {
            return t.get();
        }
        return null;
    ...
    Timestamp<T> last(){
        LocalDateTime now = LocalDateTime.now();
        Optional<Timestamp<T>> t = list.stream().filter(i -> i.getTime().isBefore(now)).sorted(Comparator.reverseOrder()).findFirst();
        if (t.isPresent()) {
            return t.get();
        }
        return null;
